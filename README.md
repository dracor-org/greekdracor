...
### Enhancements
The rebooted GreekDraCor is based on the latest version of the files from the [Perseus Digital Library](https://github.com/PerseusDL) and includes a transformation pipeline from the PDL GitHub repository to the GreekDraCor GitHub repository. During the transformation, the PDL files are particularly enriched with information on the dramatic characters (sex, role, relations, Wikidata ID). Moreover, we have employed a new method to generate the co-occurrence network data from the TEI markup for GreekDraCor for more precise network data. We revised the files starting from 25 April 2026 and merged the rebooted files on 25 August 2026 (see [here](https://github.com/dracor-org/greekdracor/pull/49)). 

###  Further Remarks
The GreekDraCor version from June 2020 to July 2026 provided automatic metrical markup by Dmitry S. Krylov. That metrical markup may be accessed via the git commit history. Note that the metrical markup has been developed with a focus on the iambic trimeter in tragedy and included quite a few undefined syllables. Thus, it might need further revision for scholarly use. More information on the metrical markup may be found in the related bachelor’s thesis:  
